...
const
  ...
  SCHEME_NAME = 'goods_k2';

// Константые значения
   AppTitle            = 'CovidMed';
   AppVersion          = ' v 1.0.0';
   AppSecretWord       = 'GoodsKeeper_Application';
   ConLogin            = '2E190C3B0A3E080C1E'; //пг юзер
   ConPass             = '125F575640795C5D4510';    // пг пароль
...
//resourcestring
  ...
  FNC_USER_INFO       = 'select * from ' + SCHEME_NAME + '.p_get_user_info(:login, :pass)';
  FNC_GET_ACCESS      = 'select * from ' + SCHEME_NAME + '.p_get_real_access(:user_id)';
  FNC_INSERT_CONTROL  = 'select ' + SCHEME_NAME + '.p_insert_control(:user_id, :fmname, :formname, :name, :caption, :fullname)';

  QR_FORMCONTROLS = 'select distinct fc.fmname, fc.formname ' + #13#10 +
                    'from ' + SCHEME_NAME + '.formcontrols fc ' + #13#10 +
                    'order by 2';
  FNC_USERS_LIST = 'select p.*' + #13#10 +
                   'from ' + SCHEME_NAME + '.p_get_user_list() p' + #13#10 +
                   'order by p.fio';
  QR_USER_INSERT = 'insert into ' + SCHEME_NAME + '.users(id, fio, login, pass, tn, active, job, is_role)' + #13#10 +
                   'values (:id, :fio, :login, :pass, :tn, 1, :job, 0)';
  QR_USER_UPDATE = 'update ' + SCHEME_NAME + '.users u' + #13#10 +
                   'set fio = :fio,' + #13#10 +
                       'login = :login,' + #13#10 +
                       'pass = :pass,' + #13#10 +
                       'tn = :tn,' + #13#10 +
                       'job = :job' + #13#10 +
                   'where u.id = :id';
  FNC_USER_ROLE_LIST = 'select p.*' + #13#10 +
                       'from ' + SCHEME_NAME + '.p_get_user_role(:id) p' + #13#10 +
                       'order by p.name';
  QR_USER_ROLE_INSERT = 'insert into ' + SCHEME_NAME + '.usergroup(user_id, role_id)' + #13#10 +
                        'values(:user_id, :id)';
  QR_USER_ROLE_DELETE = 'delete from ' + SCHEME_NAME + '.usergroup ug' + #13#10 +
                        'where ug.user_id = :user_id' + #13#10 +
                          'and ug.role_id = :id';
  FNC_ROLES_LIST = 'select * from ' + SCHEME_NAME + '.p_get_roles_list()';
  QR_ROLE_INSERT = 'insert into ' + SCHEME_NAME + '.users(id, fio, active,  is_role)' + #13#10 +
                   'values (:id, :name, 1, 1)';
  QR_ROLE_UPDATE = 'update ' + SCHEME_NAME + '.users u' + #13#10 +
                   'set fio = :name' + #13#10 +
                   'where u.id = :id';
  FNC_ROLE_USER_LIST = 'select p.*' + #13#10 +
                       'from ' + SCHEME_NAME + '.p_get_role_user(:id) p' + #13#10 +
                       'order by p.fio';

  QR_USERS_ROLES = 'select *' + #13#10 +
                   'from ' + SCHEME_NAME + '.users u' + #13#10 +
                   'where u.active = 1' + #13#10 +
                   'order by u.is_role desc, u.fio';

  FNC_ACCESS_LIST   = 'select * from ' + SCHEME_NAME + '.p_get_user_access(:id, :fmname)';
  FNC_ACCESS_UPDATE = 'select ' + SCHEME_NAME + '.p_change_user_access(:user_id, :fmid, :enable, :visible, :readonly)';
  FNC_ACCESS_ID     = 'select * from ' + SCHEME_NAME + '.p_get_user_access(:user_id, :fmname) f' + #13#10 +
                      'where f.fmid = :fmid';

  FNC_REPORTS_LIST = '';//'select null::integer as id, null::varchar as keyword where :keyword = '''' and 1=2';
                     //'select p.* from ' + CURRENT_SCHEME + '.p_reports_list(:keyword) p';
  QR_REPORTS_MODIFY = 'update ' + SCHEME_NAME + '.reports r' + #13#10 +
                      'set data = :data' + #13#10 +
                      'where r.id = :id';
  ...
  SQ_USERS = '' + SCHEME_NAME + '.users_id_seq';
  SQ_ROLES = '' + SCHEME_NAME + '.users_id_seq';

  MSG_NO_ORDERS = 'Вы не выбрали неодной заявки';
implementation
